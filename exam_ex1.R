...
dtf_X <- matrix(unlist(dtf_input[, -int_p]),
                nrow=dim(dtf_input[, -int_p])[1],
                byrow=FALSE)

dtf_y = unlist(dtf_input$cnt)

# pos_scaler <- function(lst_el) {
#   return((lst_el - mean(lst_el))/sd(lst_el))
# }
# 
# dtf_X <- matrix(nrow = dim(dtf_X_temp)[1],
#                 ncol = dim(dtf_X_temp)[2])
# 
# for (int_num in (1:dim(dtf_X_temp)[2])) {
#   dtf_X[, int_num] <- pos_scaler(lst_el = dtf_X_temp[, int_num])
# }
# 
# dtf_y <- pos_scaler(lst_el = dtf_y_temp)
...
pdf("ex1_fig9_7.pdf",family="Times",height=1.75,width=5)
